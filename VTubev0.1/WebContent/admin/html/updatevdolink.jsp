...
<html lang="en">
	<head>
		<c:set value="${requestScope.updateVdo}" var="updateVdo"></c:set>
	...
	<body class="no-skin">
									...
									<div class="widget-main padding-8">
										...
										<div class="form-group">
											...
											<div class="col-sm-12">
													...
													<label>
													<c:choose>
														<c:when test="${updateVdo.video_status==0 }">
															<input id="video-status" name="video-status" class="ace ace-switch ace-switch-6" type="checkbox" value="0">
															<span id ="tooltips-status" class="lbl tooltip-error" data-rel="tooltip"  title="Status is Deactivated !"></span>
														</c:when>
														<c:otherwise>
															<input id="video-status" name="video-status" class="ace ace-switch ace-switch-6" type="checkbox" value="1" checked>
															<span id ="tooltips-status" class="lbl tooltip-info" data-rel="tooltip"  title="Status is Activated !"></span>
														</c:otherwise>
										...
										<div class="form-group">
											...
											<div class="col-sm-12">
												...
												<input type="text" id="video-link" placeholder="http://youtube.com/watch?v=...." class="form-control" value="${updateVdo.video_url }">
										...
										<div class="form-group">
											...
											<div class="col-md-offset-10 col-md-2">
														...
														<button class="btn btn-info" type="button" id="update-video-link">
		...
		var category_id=0;
			jQuery(function($) {
				...
				$("#video-desc").val("${updateVdo.video_description }");
				...
				//LIST OF COMBOX
					...
					$.ajax({
						...
						method:"GET",
						data:{
							main_cate_id:${updateVdo.main_cate_id}
						},
				...
				function listSubOfMain(data){
					...
					for(var i=0;i<data.length;i++){
						if(${updateVdo.category_id}==data[i].category_id){
							opt+="<option value="+data[i].category_id+" selected>"+data[i].category_name+"</option>";
						}else{
				...
				function listMainCate(data){
					...
					for(var i=0;i<data.length;i++){
						if(${updateVdo.main_cate_id}==data[i].main_cate_id){
							opt+="<option value="+data[i].main_cate_id+" selected>"+data[i].main_cate_name+"</option>";
						}else{
				...
				$("#update-video-link").on("click",function(){
					if($("input[name='video-status']:checked").length>0){
						check=1;
					}else{
						check=0;
					}
					 $.ajax({
						...
						success:function(status){
							...
							if(status=="success"){
								...
								swal("Oops..!","This operation is rejected","error",function(){
									location.href="listvdo";
								});
							}
